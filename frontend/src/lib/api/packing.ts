...
export class PackingApiClient extends BaseApiClient {
  ...
  async listItems(
    tripId: number,
    listId: number,
    filters?: { category?: string; is_packed?: boolean; search?: string },
  ) {
    const params = new URLSearchParams();
    if (filters?.category) params.append('category', filters.category);
    if (filters?.is_packed !== undefined)
      params.append('is_packed', String(filters.is_packed));
    if (filters?.search) params.append('search', filters.search);
    const qs = params.toString();
    const response = await fetch(
      `${this.url(tripId)}/${listId}/items/${qs ? `?${qs}` : ''}`,
      {
        ...this._requestConfiguration(true),
        method: 'GET',
      },
  ...
  async updateItem(
    ...
    const response = await fetch(
      `${this.url(tripId)}/${listId}/items/${itemId}/`,
      {
        ...this._requestConfiguration(true),
        method: 'PATCH',
        body: JSON.stringify(data),
  ...
  async togglePacked(tripId: number, listId: number, itemId: number) {
    const response = await fetch(
      `${this.url(tripId)}/${listId}/items/${itemId}/toggle/`,
      {
        ...this._requestConfiguration(true),
        method: 'POST',
      },
