  ...
  const validateForm = () => {
    ...
    try {
      ...
      const uploadData = {
        ...
        category: formData.category && formData.category !== 'none' ? parseInt(formData.category) : undefined,
        custom_tags: customTags.length > 0 ? customTags : undefined,
        auto_delete_after_trip: formData.autoDeleteAfterTrip,
        delete_days_after_trip: formData.autoDeleteAfterTrip ? formData.deleteDaysAfterTrip : undefined,
      };
      
      console.log('Upload data:', uploadData);

      const result = await documentsApiClient.createDocument(selectedTrip.id, uploadData);
      ...
      setErrors(prev => ({ ...prev, submit: 'Failed to upload document. Please try again.' }));
    } finally {
      ...
      <div className="container mx-auto p-6 max-w-2xl">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-6">
            You don't have any trips yet. Create a trip first to upload documents.
          </p>
        </div>
      </div>
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="shared">Shared - Visible to all trip members</SelectItem>
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            <Switch
              ...
              onCheckedChange={(checked) => handleInputChange('autoDeleteAfterTrip', checked)}
            />
          ...
          {formData.autoDeleteAfterTrip && (
            <div className="space-y-2 ml-6">
              ...
              <Input
                ...
                onChange={(e) => handleInputChange('deleteDaysAfterTrip', parseInt(e.target.value))}
                className="w-32"
              />
              {errors.deleteDaysAfterTrip && (
                <p className="text-sm text-red-600 dark:text-red-400">{errors.deleteDaysAfterTrip}</p>
              )}
            </div>
          )}
        </div>
