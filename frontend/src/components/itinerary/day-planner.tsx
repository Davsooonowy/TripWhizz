...
export default function DayPlanner({ tripId }: { tripId?: string }) {
  ...
  const handleMouseDown: React.MouseEventHandler<HTMLDivElement> = (e) => {
    ...
    const onMove = (ev: MouseEvent) => {
      setCreateRange((prev) => {
        ...
        return {
          start: prev.start,
          end: Math.max(nextEnd, prev.start + minuteStep),
        };
      });
    };
    const onUp = () => {
  ...
  const selectionStyle = useMemo(() => {
    ...
    const height =
      (Math.abs(createRange.end - createRange.start) / 60) * pxPerHour;
    return {
  ...
  return (
    <div className="container max-w-5xl mx-auto px-4 py-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
        ...
        <div className="order-1 md:order-2 text-center md:text-left">
          ...
          <p className="text-muted-foreground text-sm">
            Drag on the timeline to create an event
          </p>
        </div>
          ...
          >
            <div className="relative" style={{ height: gridHeight }}>
              ...
              {events.map((ev) => {
                ...
                const height =
                  ((ev.endMinutes - ev.startMinutes) / 60) * pxPerHour;
                return (
                  <div
                    key={
                      ev.id ?? `${ev.title}-${ev.startMinutes}-${ev.endMinutes}`
                    }
                    ...
                    <div className="p-2 text-xs text-white">
                      <div className="font-semibold flex items-center gap-2">
                        ...
                        <span className="text-[11px] opacity-90">
                          ...
                          {minutesToTimeLabel(ev.endMinutes)}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="border rounded-xl p-4">
            ...
            {events.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No events yet. Drag on the timeline to add one.
              </p>
            ) : (
      ...
      <Dialog
        open={isCreating}
        onOpenChange={(open) => (!open ? cancelCreate() : setIsCreating(true))}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-3">
            ...
            {createRange && (
              <div className="text-sm text-muted-foreground">
                {minutesToTimeLabel(
                  Math.min(createRange.start, createRange.end),
                )}{' '}
                –{' '}
                {minutesToTimeLabel(
                  Math.max(createRange.start, createRange.end),
                )}
              </div>
            )}
          </div>
      ...
      <Dialog
        open={!!editingEvent}
        onOpenChange={(open) => (!open ? setEditingEvent(null) : null)}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input
                ...
                value={
                  editingEvent
                    ? minutesToTimeLabel(editingEvent.startMinutes)
                    : ''
                }
                min="00:00"
                max="23:59"
              ...
              <Input
                ...
                value={
                  editingEvent
                    ? minutesToTimeLabel(
                        editingEvent.endMinutes >= 24 * 60
                          ? 24 * 60 - 1
                          : editingEvent.endMinutes,
                      )
                    : ''
                }
                min="00:00"
          ...
          <DialogFooter className="flex flex-col md:flex-row gap-2 md:gap-2">
            ...
            <Button className="w-full md:w-auto" onClick={saveEditingEvent}>
              Save
            </Button>
          </DialogFooter>
        </DialogContent>
