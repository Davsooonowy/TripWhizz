...
export default function ActivitiesList({ tripId }: { tripId?: string }) {
  ...
  if (!resolvedTripId) {
    return <div className="container max-w-4xl mx-auto px-4 py-6">Select or open a trip to view activities.</div>;
  }
  ...
  return (
    <div className="container max-w-4xl mx-auto px-4 py-6">
      ...
      {events === null ? (
        ...
        <div className="text-sm text-muted-foreground">No activities yet. Create events in Days to see them here grouped by date.</div>
      ) : (
          ...
          {Object.entries(grouped).map(([date, dayEvents]) => (
            <Card key={date}>
              ...
              <CardContent className="space-y-2">
                {dayEvents.map((ev) => (
                  <div key={ev.id} className="flex items-start justify-between gap-4 border rounded-md p-3 cursor-pointer hover:bg-muted/50" onClick={() => setEditing(ev)}>
      ...
      <Dialog open={!!editing} onOpenChange={(open) => (!open ? setEditing(null) : null)}>
        <DialogContent>
          ...
          {editing && (
            <div className="space-y-3">
              <Input
                ...
                value={editing.title}
                onChange={(e) => setEditing((prev) => prev ? { ...prev, title: e.target.value } as ItineraryEventDto : prev)}
              />
              <div className="grid grid-cols-2 gap-3">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setEditing((prev) => prev ? { ...prev, start_minutes: Math.min(mins, (prev.end_minutes ?? 0) - 1) } : prev);
                  }}
                />
                <Input
                  type="time"
                  value={minutesToTimeLabel(editing.end_minutes >= 24 * 60 ? 24 * 60 - 1 : editing.end_minutes)}
                  min="00:00"
                  max="23:59"
                  step={minuteStep * 60}
                  className="bg-background focus:bg-muted"
                  onChange={(e) => {
                    const [h, m] = e.target.value.split(':').map((x) => parseInt(x, 10));
                    const mins = clamp(h * 60 + m, 0, 23 * 60 + 59);
                    setEditing((prev) => prev ? { ...prev, end_minutes: Math.max(mins, (prev.start_minutes ?? 0) + minuteStep) } : prev);
                  }}
                />
              </div>
              <DatePicker
          ...
          <DialogFooter className="flex flex-col md:flex-row gap-2 md:gap-2">
            <Button className="w-full md:w-auto" variant="secondary" onClick={() => setEditing(null)}>Cancel</Button>
            <Button className="w-full md:w-auto" variant="destructive" onClick={() => deleteEvent(editing?.id)}>Delete</Button>
            <Button className="w-full md:w-auto" onClick={saveEditing}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
