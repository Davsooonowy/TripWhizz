...
export function TripInvitationDialog({ open, onOpenChange, notification, onResponse }: TripInvitationDialogProps) {
  ...
  const handleResponse = async (action: "accept" | "reject") => {
    ...
    try {
      ...
      await tripsApiClient.respondToInvitation(notification.related_object_id, action)
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 py-4">
          <div className="text-center space-y-4">
            ...
            <div>
              <h3 className="text-lg font-semibold">{tripName || "Trip Invitation"}</h3>
              <p className="text-muted-foreground">You've been invited to join this trip</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 p-4 bg-muted/30 rounded-lg">
            <Avatar className="h-10 w-10">
              <AvatarImage
                ...
                alt={getInviterName()}
              />
              <AvatarFallback className="bg-primary text-white">{getInitials(getInviterName())}</AvatarFallback>
            </Avatar>
        ...
        <DialogFooter className="flex gap-2 sm:gap-2">
          ...
          >
            {isResponding ? <LoadingSpinner size="sm" /> : <X className="h-4 w-4" />}
            Decline
          </Button>
          <Button onClick={() => handleResponse("accept")} disabled={isResponding} className="flex-1 gap-2">
            {isResponding ? <LoadingSpinner size="sm" /> : <Check className="h-4 w-4" />}
            Accept
          </Button>
        </DialogFooter>
      </DialogContent>
