...
export default function PackingItemsPage() {
	...
	const api = React.useMemo(() => new PackingApiClient(authenticationProviderInstance), []);
		...
		<div className="container mx-auto p-6 max-w-4xl">
			...
			<Card className="mb-8">
				...
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
						...
						<div>
							<Input
								...
								onChange={(e) =>
									setNewItem((s) => ({ ...s, quantity: parseInt(e.target.value) || 1 }))
								}
					...
					<Button onClick={addItem} className="mt-4" disabled={!newItem.name.trim() || isLoadingItems}>
									...
									>
										...
										<div className="flex-1">
											<div className="flex items-center gap-2">
												<span className={`font-medium ${item.is_packed ? 'line-through text-muted-foreground' : ''}`}>
													{item.name}
												</span>
												{item.quantity > 1 && (
			...
			{!isLoadingItems && items.length === 0 && (
				<Card className="text-center py-12">
					<CardContent>
						...
						<h3 className="text-lg font-semibold mb-2">Your packing list is empty</h3>
						<p className="text-muted-foreground mb-4">
							Start adding items to your packing list to get organized for your trip
						</p>
						<Button onClick={() => setNewItem((s) => ({ ...s, name: 'Passport' }))}>
							<Plus className="h-4 w-4 mr-2" />
							Add Your First Item
						</Button>
					</CardContent>
