  ...
  const onCreateSettlement = async () => {
    try {
      ...
      if (!payerId || !payeeId || payerId === payeeId) {
        toast({ title: 'Validation', description: 'Please select two different participants.', variant: 'destructive' });
        return;
      }
      if (amount <= 0) {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <Label>Payer</Label>
              <Select value={String(payerId)} onValueChange={(v) => setPayerId(Number(v))}>
            ...
            <div>
              <Label>Payee</Label>
              <Select value={String(payeeId)} onValueChange={(v) => setPayeeId(Number(v))}>
                <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>
                  {participants.map((p) => (
                    <SelectItem key={p.id} value={String(p.id)}>{p.username}</SelectItem>
          ...
          <div className="space-y-3">
            ...
            {settlements.length > 5 && (
              <div className="flex justify-center pt-2">
                <Button variant="ghost" size="sm" onClick={() => setShowAllSettlements((v) => !v)}>
